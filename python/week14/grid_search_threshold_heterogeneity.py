...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Grid search over threshold-voltage heterogeneity variances for the "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "week14" / "results" / "threshold_voltage_heterogeneity_grid_search.csv",
...
def build_heterogeneity(var_ss4, var_b, truncate_std):
    return {
        ...
        "b": {
            6: ("gaussian", None, float(var_b), float(truncate_std)),
        },
    }
