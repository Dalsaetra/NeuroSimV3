...
DEFAULT_VARIANCES = (0.0, 1.0, 2.0, 4.0, 7.0, 10.0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Grid search over peak-voltage heterogeneity variances for the "
            "week14 setup, across spatial and fixed-indegree E/I topologies."
        )
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "week14" / "results" / "peak_voltage_heterogeneity_grid_search.csv",
        help="CSV path for per-trial results.",
    )
    parser.add_argument(
        "--topologies",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="Variance values for Gaussian peak-voltage heterogeneity.",
    )
    parser.add_argument("--seed-start", type=int, default=1234, help="First topology seed.")
    parser.add_argument("--seed-count", type=int, default=5, help="Number of topology seeds per condition.")
    ...
    parser.add_argument("--sim-ms", type=float, default=3000.0, help="Simulation duration in ms.")
    parser.add_argument("--input-rate-hz", type=float, default=50.0, help="Poisson input rate in Hz.")
    parser.add_argument("--input-amplitude", type=float, default=0.5, help="Poisson input amplitude.")
    parser.add_argument(
        "--truncate-std",
        type=float,
        default=2.0,
...
def build_heterogeneity(var_ss4, var_b, truncate_std):
    return {
        ...
        "b": {
            7: ("gaussian", None, float(var_b), float(truncate_std)),
        },
    }
...
def collect_simulation_metrics(
    ...
    input_mask = build_input_mask(connectome.neuron_population)
    ...
    stats.inhibitory_mask = connectome.neuron_population.inhibitory_mask.copy()
    stats.Vs.append(sim.neuron_states.V.astype(np.float32, copy=True))
    stats.spikes.append(sim.neuron_states.spike.copy())
    stats.ts.append(sim.t_now)

    poisson = PoissonInput(
        connectome.neuron_population.n_neurons,
        rate=input_rate_hz * input_mask,
        amplitude=input_amplitude,
        rng=poisson_rng,
    )

    n_steps = int(sim_ms / dt_ms)
    for _ in range(n_steps):
        sim.step(spike_ext=poisson(dt_ms), reward=1.0)
        stats.Vs.append(sim.neuron_states.V.astype(np.float32, copy=True))
        stats.spikes.append(sim.neuron_states.spike.copy())
        stats.ts.append(sim.t_now)
...
def main():
    ...
    print(
        ...
        f"{total_trials} assigned trials, output={output_path}",
        flush=True,
    )

    try:
        for trial in job_trials:
            topology_idx = trial["topology_idx"]
            topology_name = trial["topology_name"]
            var_ss4_idx = trial["var_ss4_idx"]
            var_ss4 = trial["var_ss4"]
            var_b_idx = trial["var_b_idx"]
            var_b = trial["var_b"]
            ...
            key = (
                topology_name,
                str(topology_seed),
                format_float_key(var_ss4),
                format_float_key(var_b),
            )
            ...
            _, connectome = build_connectome_from_graph(
                G,
            ...
            row = {
                ...
                "topology_seed": topology_seed,
                "variance_ss4_vp": format_float_key(var_ss4),
                "variance_b_vp": format_float_key(var_b),
                "truncate_std": args.truncate_std,
                "heterogeneity_param_index": 7,
                "heterogeneity_param_name": "Vp",
                "n_neurons": args.n_neurons,
            ...
            del G, connectome, metrics, row, pop_rng, init_rng, poisson_rng
            gc.collect()
    finally:
