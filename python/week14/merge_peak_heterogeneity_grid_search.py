...
DEFAULT_OUTPUT = PROJECT_ROOT / "week14" / "results" / "peak_voltage_heterogeneity_grid_search.csv"
JOB_FILE_RE = re.compile(r"^(?P<stem>.+)\.job(?P<job>\d+)of(?P<total>\d+)(?P<suffix>\.[^.]+)$")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Merge per-job peak heterogeneity grid search CSV files into one CSV."
    )
    parser.add_argument(
        "--input-base",
        type=Path,
        ...
        help=(
            ...
            "Example: week14/results/peak_voltage_heterogeneity_grid_search.csv"
        ),
    )
    parser.add_argument(
        "--output",
...
def trial_key(row):
    return (
        ...
        row.get("variance_ss4_vp", ""),
        row.get("variance_b_vp", ""),
    )
...
def merge_job_files(job_files):
    ...
    merged_rows.sort(
        key=lambda row: (
            ...
            float(row.get("variance_ss4_vp", "0")),
            float(row.get("variance_b_vp", "0")),
        )
    )
