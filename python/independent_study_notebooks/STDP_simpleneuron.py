...
times_to_plot = [1000, 10000, 30000, 60000, 100000, 150000, 200000, 250000, 299000, 600000, 1000000, 2000000, 3000000, 3590000] # ms
seconds_to_run = 3600
...
for i in tqdm(range(int(seconds_to_run * 1000 // dt))):
    ...
    if sim.t_now in times_to_plot:
        sim.plot_spike_raster(figsize=(10, 6), title=f"Spike raster at t={sim.t_now} ms", save_path=f"stdp_simplemodel/spike_raster_ws_{weight_scale}_g{g}_d{delay_mean}_std{delay_std}_t{sim.t_now}.png")
        sim.reset_stats()
