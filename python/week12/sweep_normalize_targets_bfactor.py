...
DEFAULT_IN_MIN = 1.0
DEFAULT_IN_MAX = 500.0
DEFAULT_B_FACTOR_MIN = 0.05
DEFAULT_B_FACTOR_MAX = 1.0
...
def run_single_combo(combo: tuple[float, float, float]) -> dict[str, float | str]:
    in_e_ss4, in_i_ss4, b_factor = combo
    in_e_b = float(in_e_ss4) * float(b_factor)
    in_i_b = float(in_i_ss4) * float(b_factor)

    row: dict[str, float | str] = {
        "normalize_target_in_E_ss4": float(in_e_ss4),
        "normalize_target_in_I_ss4": float(in_i_ss4),
        "b_factor": float(b_factor),
        "normalize_target_in_E_b": float(in_e_b),
        "normalize_target_in_I_b": float(in_i_b),
    }
    t0 = time.perf_counter()

    try:
        G = _build_network(
            normalize_target_in_E={"ss4": float(in_e_ss4), "b": float(in_e_b)},
            normalize_target_in_I={"ss4": float(in_i_ss4), "b": float(in_i_b)},
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep ss4 in-targets and a shared b-factor on a logarithmic 10x10x10 grid."
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parent / "week12_normalize_target_bfactor_sweep.csv",
    ...
    parser.add_argument(
        ...
        help="Number of logarithmic points per swept dimension.",
    )
    parser.add_argument(
        "--in-min",
        type=float,
        default=DEFAULT_IN_MIN,
        help="Minimum ss4 in-target value.",
    )
    parser.add_argument(
        "--in-max",
        type=float,
        default=DEFAULT_IN_MAX,
        help="Maximum ss4 in-target value.",
    )
    parser.add_argument(
        "--b-factor-min",
        type=float,
        default=DEFAULT_B_FACTOR_MIN,
        help="Minimum multiplicative factor used for b targets.",
    )
    parser.add_argument(
        "--b-factor-max",
        type=float,
        default=DEFAULT_B_FACTOR_MAX,
        help="Maximum multiplicative factor used for b targets.",
    )
...
def main() -> None:
    ...
    if args.in_min <= 0 or args.in_max <= 0:
        raise ValueError("in-min and in-max must be > 0 for logarithmic spacing")
    if args.b_factor_min <= 0 or args.b_factor_max <= 0:
        raise ValueError("b-factor-min and b-factor-max must be > 0 for logarithmic spacing")

    in_values = tuple(float(v) for v in np.geomspace(args.in_min, args.in_max, args.num_points))
    b_factor_values = tuple(float(v) for v in np.geomspace(args.b_factor_min, args.b_factor_max, args.num_points))
    combos = list(itertools.product(in_values, in_values, b_factor_values))
